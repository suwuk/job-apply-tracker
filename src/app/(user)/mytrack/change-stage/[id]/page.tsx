...
import { 
  ...
  ClipboardList, FileX, Calendar, Star 
} from "lucide-react";
import { 
  ApplicationStatus, 
  EmploymentType,
      ...
      if (form.status !== currentJob.status) {
        if (form.status === ApplicationStatus.REJECTED) {
          ...
          updatedStages.push({
            ...
            completedAt: ["offer", "accepted"].includes(form.status) ? now : null,
        ...
        <div className="bg-white rounded-3xl p-5 md:p-10 shadow-sm border border-slate-100 space-y-8">
                ...
                <div className="relative">
                  <select
                    ...
                    className={`w-full h-14 pl-5 pr-12 rounded-2xl border appearance-none outline-none font-bold text-sm transition-all shadow-sm ${
                      form.status === "rejected" ? "bg-red-50 border-red-200 text-red-700" : 
                      ["offer", "accepted"].includes(form.status) ? "bg-emerald-50 border-emerald-200 text-emerald-700" : 
                      "bg-blue-50 border-blue-200 text-blue-700"
                    }`}
                  >
                    {Object.values(ApplicationStatus).map(s => (
                      <option key={s} value={s} className="text-black">{s.toUpperCase()}</option>
                    ))}
                  </select>
                  <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-900" size={20} />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select
                    ...
                    className={`w-full h-14 pl-5 pr-12 rounded-2xl border appearance-none outline-none font-bold text-sm transition-all shadow-sm ${getPriorityColor(form.priority)}`}
                  >
                    {Object.values(ApplicationPriority).map(p => (
                      <option key={p} value={p} className="text-black">{p.toUpperCase()}</option>
                    ))}
                  </select>
                  <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-900" size={20} />
                </div>
                ...
                <div className="relative">
                  ...
                  >
                    {Object.values(EmploymentType).map(e => (
                      <option key={e} value={e} className="text-black">{e.replace("_", " ").toUpperCase()}</option>
                    ))}
                  </select>
                  <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-900" size={20} />
                </div>
          ...
          {isStatusChanged && showStageInput && (
            <div className={`p-5 md:p-8 rounded-2xl border-2 border-dashed animate-in slide-in-from-top-4 duration-500 ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-600">Stage Title</label>
                  <input
                ...
                <div className="md:col-span-2 space-y-2">
                  <label className="text-xs font-bold text-slate-600">Stage Notes</label>
                  <textarea
                    value={form.stageNotes}
                    onChange={e => setForm({...form, stageNotes: e.target.value})}
                    placeholder="Add meeting links or notes..."
                    className="w-full p-4 bg-white border border-slate-200 rounded-xl outline-none text-sm min-h-[100px] resize-none text-slate-900 font-medium focus:border-blue-500 transition-colors"
                  />
                </div>
              </div>
            </div>
          ...
          {isStatusChanged && form.status === ApplicationStatus.REJECTED && (
            <div className="p-5 md:p-8 bg-red-50/40 border-2 border-dashed border-red-100 rounded-2xl animate-in slide-in-from-top-4 duration-500">
              ...
              <div className="grid grid-cols-1 gap-5">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-600">Rejection Date</label>
                  <input
                    type="datetime-local"
                    value={form.rejectedAt}
                    onChange={e => setForm({...form, rejectedAt: e.target.value})}
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full p-4 bg-white border border-red-200 rounded-xl outline-none text-sm min-h-[100px] resize-none text-slate-900 font-medium focus:border-red-500 transition-colors"
                  />
                </div>
              </div>
            </div>
          ...
          <section className="space-y-3">
            ...
            <textarea
              ...
              className="w-full p-5 md:p-8 bg-slate-50 border border-slate-200 rounded-3xl outline-none min-h-[180px] text-slate-900 text-sm md:text-base font-semibold transition-all focus:bg-white focus:ring-4 focus:ring-slate-100"
            />
          </section>
        </div>
